...
def fit_x_y_curve(x, y, func, p0, bounds):
    ...
    if x.dropna().empty or y.dropna().empty:
        ...
        popt, _ = curve_fit(func, tmp.iloc[:, 0], tmp.iloc[:, 1], p0=p0, maxfev=10000, bounds=bounds)
...
popt, r_squared = fit_x_y_curve(apar[idx_day], ql_day, ql_par_func, p0, bounds)
...
# initial_guess = [0.01, 0.05, 0.4, 0.025]
# bounds = (
#     [ 1e-5, 1e-5, 0.1,  0.0],   # 下界 (限制d>=0，确保温度越高天花板越高)
#     [ 0.1,  0.5,  1.0,  0.08]   # 上界
# )
...
# qL_fitted1 = qL_model_exponential((par_demo, t_demo), 0.0053956, 0.0830504) # 

fig, ax = plt.subplots(figsize=(8, 6))
ax.scatter(df_matched.loc[idx_day, 'PAR (µmol/m2/s)'], ql[idx_day], 
           c = df_matched.loc[idx_day,'Ta (°C)'], cmap='viridis', s=20, edgecolor='none')
ax.scatter(par_demo, qL_fitted, s=20, edgecolor='k', facecolor='none', label = f'Fitted parameters: a={popt[0]:.3f}, b={popt[1]:.5f}')
# ax.scatter(par_demo, qL_fitted1, s=20, edgecolor='none', label='Reference parameters: qL_min=0.2, a=0.003, b=0.000287')
# ax.scatter(apar[idx_day], ql_fit, color='red', marker = '.', label=f'Fitted parameters: alpha={alpha_ql:.3f}, beta={beta_ql:.5f}, hf_par={hf_par_fit:.1f}, R²={r_squared_qL:.3f}')
# ax.plot(par_range, ql_dir, color='blue', linestyle='--', label='Typical qL-PAR curve (dir)')
# ax.plot(par_range, ql_dif, color='green', linestyle='--', label='Typical qL-PAR curve (dif)')
